...
def load_dataset(dataset):
    ...
    path_dataset = "data-workload/"

    # check the initialized schema manually first.
    df = pd.read_csv(path_dataset + dataset +"/original.csv")

    # clean the data according to the data set name
    if dataset == 'tpcds':
        df = df.drop('Unnamed: 0', axis=1)
    elif dataset == 'census13':
